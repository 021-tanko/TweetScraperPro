...
class tweetspy:
    ...
    def save_pic(self,picture):
        if not os.path.exists('tweetspy_img'):
            os.makedirs('tweetspy_img')
        if not os.path.exists('tweetspy_img/{0.author}'.format(self)):
            os.makedirs('tweetspy_img/{0.author}'.format(self))
        filename = picture[len('https://pbs.twimg.com/media/'):]
        save_dir = 'tweetspy_img/{0.author}'.format(self)
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(prog='tweetspy.py',usage='python %(prog)s [options]',description="tweetspy.py - An Advanced Twitter Scraping Tool")
