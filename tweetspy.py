...
async def getUrl(init):
    '''
    URL Descision:
    TweetSpy utilizes positions of Tweet's from Twitter's search feature to
...
async def getFeed(init):
    ...
    try:
        ...
        # TweetSpy will realize that it's done scraping.
        pass
...
async def outTweet(tweet):
    ...
    '''
    There were certain cases where I used TweetSpy
...
async def main():
    ...
    while True:
        ...
        with, telling TweetSpy it's finished scraping.
        '''
        if len(feed) > 0:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(prog="tweetspy.py", usage="python3 %(prog)s [options]", description="tweetspy.py - An Advanced Twitter Scraping Tool")
