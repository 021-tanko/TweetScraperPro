...
def _output(obj, output, config, **extra):
    ...
    if config.Lowercase:
        if isinstance(obj, str):
            obj = obj.lower()
        elif str(type(obj)) == "<class 'tweetscraperpro.user.user'>":
            pass
        else:
...
async def Tweets(tw, location, config, conn):
    ...
    if copyright is None and is_tweet(tw):
        ...
        if datecheck(tweet.datestamp, config):
            ...
            if config.Store_object:
                tweets_object.append(tweet) #tweetscraperpro.tweet.tweet
...
async def Users(u, config, conn):
    ...
    if config.Store_object:
        user_object.append(user) # tweetscraperpro.user.user
