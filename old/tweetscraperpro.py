...
async def getUrl(init):
    '''
    URL Descision:
    TweetScraperPro utilizes positions of Tweet's from Twitter's search feature to
...
def err(e, func):
    ...
    print(msg, file=open("tweetscraperpro_debug.log", "a", encoding="utf-8"))
    print(msg)
...
async def main():
    ...
    if action != "":
        ...
        while _since < _until:
            ...
            with, telling TweetScraperPro it's finished scraping.
            '''
            if len(feed) > 0:
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(prog="tweetscraperpro.py", usage="python3 %(prog)s [options]", description="tweetscraperpro.py - An Advanced Twitter Scraping Tool")
